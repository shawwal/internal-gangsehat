...
export default function SportMassagePerformaPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Gagal memuat data performa terapis sport massage.')
    } finally {
  ...
  function handleExportExcel() {
    ...
    exportToExcel(rows, cols, `performa_sport_massage_${range.from}_${range.to}`)
  }

  function handleExportPdf() {
    openPrintableReport({
      title: 'Performa Terapis Sport Massage',
  ...
  if (branchEnabled === false && !isDirector) {
    return <div className="glass-card p-8 text-sm text-muted-foreground">Sport Massage belum diaktifkan untuk cabang ini.</div>
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground">Performa Terapis Sport Massage</h1>
            <p className="text-sm text-muted-foreground">{branchName} · rekap sesi hadir sport massage</p>
          </div>
        </div>
