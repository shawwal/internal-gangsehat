...
CREATE TABLE public.internal_profiles (
  ...
  phone text,
  role USER-DEFINED NOT NULL DEFAULT 'non-staff'::internal_user_role,
  branch_id uuid,
  avatar_url text,
  is_active boolean NOT NULL DEFAULT true,
  created_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.schedule_slots (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  shift text NOT NULL CHECK (shift = ANY (ARRAY['PAGI'::text, 'SORE'::text])),
...
CREATE TABLE public.terapi_awal_assessments (
  ...
  prom_used text CHECK (prom_used = ANY (ARRAY['LEFS'::text, 'SPADI'::text, 'ODI'::text, 'Other'::text])),
  ...
  CONSTRAINT terapi_awal_assessments_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branches(id),
  CONSTRAINT terapi_awal_assessments_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.internal_profiles(id)
);
CREATE TABLE public.session_notes (
  ...
  symptom_trend text CHECK (symptom_trend = ANY (ARRAY['IMPROVING'::text, 'SAME'::text, 'WORSENING'::text])),
  subjective_notes text,
  objective_findings text,
  clinical_impression text,
  treatments_performed ARRAY NOT NULL DEFAULT '{}'::text[],
  hep_given text,
  next_plan text,
  treatment_notes text,
  CONSTRAINT session_notes_pkey PRIMARY KEY (id),
  CONSTRAINT session_notes_visit_id_fkey FOREIGN KEY (visit_id) REFERENCES public.patient_visits(id),
