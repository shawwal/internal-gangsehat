    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-2xl border border-border p-6 w-full max-w-sm shadow-xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-base font-semibold text-foreground">Nonaktifkan Pengguna</h2>
            <p className="text-xs text-muted-foreground">{target.email}</p>
          </div>
        </div>
        <p className="text-sm text-muted-foreground mb-1">
          Akun <span className="font-semibold text-foreground">{target.full_name || target.email}</span> akan dinonaktifkan dan tidak dapat login lagi.
        </p>
        <p className="text-sm font-medium text-destructive mb-5">Riwayat data (kunjungan, transaksi, dll) akan tetap tersimpan.</p>
