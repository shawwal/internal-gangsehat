...
interface Visit {
  ...
  // Supabase returns object at runtime for many-to-one FK joins
  internal_profiles: TherapistInfo | TherapistInfo[] | null
}
...
function getTherapist(raw: Visit['internal_profiles']): TherapistInfo | null {
  if (!raw) return null
      ...
      {!visits.length ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm" aria-label="Riwayat kunjungan">
            ...
            <tbody>
              {visits.map((v, i) => {
                const therapist = getTherapist(v.internal_profiles)
                return (
                  <tr
                    key={v.id}
                  ...
                  >
                    <td className="py-2.5 pr-4">
                      ...
                      {v.shift ? (
                        ...
                        <p className="text-xs text-muted-foreground truncate max-w-40">{v.chief_complaint}</p>
                      ) : null}
                    </td>
                    ...
                    <td className="py-2.5 pr-4 hidden md:table-cell">
                      {therapist ? (
                        ...
                        >
                          {therapist.nickname
                            ? <><span className="font-medium">{therapist.nickname}</span> · {therapist.full_name}</>
                            : therapist.full_name
                          }
                        </Link>
                      ) : (
